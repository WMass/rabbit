...
if __name__ == "__main__":
    ...
    for linear in (False, True):
        ...
        for tag in ("noM", "M"):
            ...
            extra = ""
            if Csr is not None:
                extra = (f"  sandwich(flat)={np.sqrt(Csr[0,0]):.4f}  "
                         f"sandwich(dif)={np.sqrt(ddif@Csr@ddif):.4f}")
            print(f"  {tag:3s} rnorm={np.round(rnorm, 4)}  "
        ...
        verdict = "DE-BIASED (sigma inflated)" if s_M > 1.2 * s_no else "no de-bias (cancelled)"
        print(f"  -> curvature sigma(dif): {s_no:.4f} -> {s_M:.4f}   {verdict}")
        if res["M"][2] is not None:
            s_sand = np.sqrt(ddif @ res["M"][2] @ ddif)
            print(f"  -> sandwich sigma(dif):  {s_sand:.4f}   "
