...
def main():
    ...
    ok &= p
    print(
        f"1. param_names                  {'PASS' if p else 'FAIL'}   "
        f"{z5.param_names}"
    )
    ...
    ok &= p
    print(
        f"2. c = 0 is bit-identical       {'PASS' if p else 'FAIL'}   "
        f"max |d| {d:.3e}"
    )
    ...
    vc = dict(v0, **{f"shape{k}": tf.constant(c[k - 1], DT) for k in range(1, 6)})
    ...
    ref = np.exp(
        np.clip(
            sum(c[k - 1] * legendre.legval(uu, [0] * k + [1]) for k in range(1, 6)),
            -30.0,
            30.0,
        )
    )
    ...
    ok &= p
    print(
        f"3. factor == exp(sum c_k P_k)   {'PASS' if p else 'FAIL'}   "
        f"max |d| {d:.3e}"
    )
    ...
    ok &= p
    print(
        f"4. pdf still normalised         {'PASS' if p else 'FAIL'}   "
        f"sum p dm - 1 = {s-1:.3e}"
    )
    ...
    with tf.GradientTape() as t:
        vv = dict(
            v0,
            shape1=v,
            shape2=tf.constant(0.0, DT),
            shape3=tf.constant(0.0, DT),
            shape4=tf.constant(0.0, DT),
    ...
    ok &= p
    print(
        f"5. gradient vs FD               {'PASS' if p else 'FAIL'}   "
    ...
    d = max(abs(np.max(np.abs(z6.pdf(vc).numpy() - z5.pdf(vc).numpy()))), 0.0)
    p = (
        tuple(z6.param_names) == tuple(z5.param_names)
        and z6.config() == z5.config()
        and d == 0.0
    )
    ok &= p
    print(
        f"6. config round trip            {'PASS' if p else 'FAIL'}   "
        f"max |d| {d:.3e}"
    )
    ...
    p = (
        all(f"shape{k}" in dec for k in range(1, 6))
        and all(dec[f"shape{k}"][3] == 0 for k in range(1, 6))
        and dec["Gamma_Z"][1] == 2.3
        and dec["Gamma_Z"][3] == 1
    )
    ok &= p
    print(
        f"7. declarations                 {'PASS' if p else 'FAIL'}   "
        f"shape1 {dec['shape1']}"
    )
