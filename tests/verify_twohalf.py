...
def numpy_ref():
    ...
    flat_A = rng.binomial(sw_flat, 0.5)
    flat_B = sw_flat - flat_A
    step_A = rng.binomial(sw_step, 0.5)
    step_B = sw_step - step_A
    ...
    def H(T):
        return np.einsum("bi,bj,b->ij", T, T, 1 / V)

    def b(T):
        return np.einsum("bi,b,b->i", T, 1 / V, data)
...
if __name__ == "__main__":
    ...
    print(
        f"  cf    r={np.round(r_cf,4)}  curv(dif)={np.sqrt(ddif@Ccurv@ddif):.4f}  "
    ...
    print(
        f"  cf    r={np.round(xr_cf,4)}  curv(dif)={np.sqrt(ddif@Cr_curv@ddif):.4f}  "
        f"sandwich(dif)={np.sqrt(ddif@Cr_sand@ddif):.4f}"
    )

    ok = (
        np.allclose(xr_cf, r_cf, atol=1e-3)
        and np.isclose(
            np.sqrt(ddif @ Cr_curv @ ddif), np.sqrt(ddif @ Ccurv @ ddif), atol=1e-3
        )
        and np.isclose(
            np.sqrt(ddif @ Cr_sand @ ddif), np.sqrt(ddif @ Csand @ ddif), atol=1e-3
        )
    )
