...
def build_one(sig, deltas, s2v, **kw):
    ...
    fam = [{"name": "hit", "param": "k_hit", "kind": "gauss"}]
    t = unbinned.MassCFTerm(
        "one", sigma=np.full(n, sig), mobs=deltas, tgrid=tgrid,
        families=fam, vgf=np.full(n, 0.3),
        kernel=unbinned.BreitWignerKernel(width_param="G", mass_param="dM"),
        m_ref=MREF, scale_param="alpha", chunk=n,
        jensen_s2=np.full(n, s2v), jensen_mode="exact", **kw)
    unbinned.declare_params(t, {"k_hit": (1.0, np.nan, 1.0, 0),
                                "G": (2.5, np.nan, 2.5, 0),
                                "dM": (0.0, np.nan, 0.0, 0),
                                "alpha": (0.0, np.nan, 0.0, 1)})
...
def main():
    ...
    p2 = np.allclose(corr[~inside], (du * m - np.sign(mobs[lo:hi]) * lim)[~inside],
                     rtol=0, atol=1e-11)
    ...
    print(f"6. corr_clip in/out/monotone    "
    ...
    ok &= p
    print(f"7. corr_clip = 0 is unclipped   {'PASS' if p else 'FAIL'}   "
          f"{nll(z0, xe)!r}")
