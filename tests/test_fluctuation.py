...
def build(
    ...
    **kw,
    ...
    t = unbinned.MassCFTerm(
        ...
        mobs=np.asarray(mobs, float),
        tgrid=tgrid,
        families=fam,
        vgf=np.asarray(vgf, float),
...
def dens(t, **over):
    return t.raw_density(
        tf.constant([float(values(t, **over)[p].numpy()) for p in t.param_names], DT)
    ).numpy()
...
def nll(t, **over):
    return float(
        t.nll(
            tf.constant(
                [float(values(t, **over)[p].numpy()) for p in t.param_names], DT
            )
        ).numpy()
    )
...
def test_off_is_identical():
    ...
    z = build(
        sigma, mobs, a_res=np.zeros(n), jensen_s2=np.zeros(n), corr_form="fluctuation"
    )
    d = nll(z)
...
def test_density_moments():
    ...
    def moments(a_val, s2_val, jensen):
        ...
        if t._fluct_active:
            t._fl_g = tf.constant(
                np.full(
                    n, (0.0 if a_val is None else -a_val) + (sig / m if jensen else 0.0)
                ),
                tf.float64,
            )
            t._fl_d = (
    ...
    for label, a_val, s2_val, jensen in (
        ...
        print(
            f"  {label}  norm {z:.10f}   mean {mu*1e3:+8.4f} MeV   "
...
def test_delta_kernel_matches_residual():
    ...
    print(
        f"  correction size: residual {d_res:+.5f} e-3, "
...
def test_bounded_at_z():
    ...
    wsup = float(
        np.max((np.abs(wr.numpy()) + np.abs(wi.numpy())) * np.exp(sre.numpy()))
    )
    print(
        f"  residual form moves the residual by median "
        f"{np.median(moved_res)*1e3:8.1f} MeV, max {moved_res.max():.3f} GeV"
    )
    print(
        f"  fluctuation form   d_i        median "
        f"{np.median(moved_flu)*1e3:8.1f} MeV, max "
        f"{moved_flu.max()*1e3:.1f} MeV"
    )
    print(
        f"  fluctuation CF factor |w| max over the whole tau grid {wmax:.4f}, "
...
def test_gradient():
    ...
    x0 = np.array(
        [1.0 if p == "k_hit" else (2.5 if p == "G" else 0.0) for p in t.param_names]
    )
    ...
    for i, p in enumerate(t.param_names):
        ...
        xp[i] += h
        xm[i] -= h
        fd = (float(t.nll(tf.constant(xp, DT))) - float(t.nll(tf.constant(xm, DT)))) / (
            2 * h
        )
...
def test_cancellation():
    ...
    print(
        f"  c median {np.median(c)*1e3:+.3f} MeV, "
...
def test_set_corrections():
    ...
    for label, kw in ladder.items():
        ...
        a, b = nll(got), nll(ref)
        print(
            f"  {label:9s} one-card {a!r}  purpose-built {b!r}  " f"identical: {a == b}"
        )
