...
from rabbit.lineshapes import ZGammaLineshape  # noqa: E402
...
def test_modifiers(args):
    ...
    zg2 = ZGammaLineshape(
        acceptance={"kind": "grid", "m": fine, "a": zb2._acceptance_on(fine)}, **kw
    )
...
def _toy_table(nodes, u_edges, lam0=50.0, u_max=7.0):
    ...
    with np.errstate(invalid="ignore", divide="ignore"):
        U = np.where(
            K > 0.0, M1 / np.where(K > 0.0, K, 1.0), 0.5 * (a + b) * np.ones_like(K)
        )
    U = np.clip(U, a, b)
    return {
        "m_nodes": nodes,
...
def test_fsr_table(args):
    ...
    print(
        f"  8a. table of the same atoms: fold matrix max rel dev {d:.2e}, "
        f"pdf {dp:.2e}"
    )
    ...
    for rule in ("interp", "mass"):
        z = ZGammaLineshape(
            fsr=_toy_table([50.0, 91.0, 200.0], ue), fsr_deposit=rule, **kw
        )
        F = z._fsr_mat.numpy()
        K, M1, p0 = z._table_rows(z.m_born)
        U = np.clip(np.where(K > 0, M1 / np.where(K > 0, K, 1), 0.0), ue[:-1], ue[1:])
        dm, m0 = z.dm, W[0]
        ref = np.zeros_like(F)
        ...
        print(
            f"  8b. all-narrow, deposit '{rule}': vs a naive deposit "
    ...
    print(
        f"  8c. all-wide vs a naive per-cell hat projection: max rel dev "
    ...
    for nm in (1024, 2048, 4096):
        ...
        for rule in ("interp", "mass"):
            ...
            out.append(
                (
                    np.max(np.abs(zr.pdf(V).numpy() - ex)) / np.max(ex),
                    zr._fsr_mat.numpy().sum(axis=0).max() - 1.0,
                )
            )
        print(
            f"  8d. nm = {nm:5d}: vs the exact fold, interp {out[0][0]:.2e} "
    ...
    z3 = ZGammaLineshape(
        fsr=_toy_table([50.0, 91.0, 200.0], _ugrid(2000)), fsr_deposit="mass", **kw
    )
    got, want = z3.fsr_column_mass()
    e = np.abs(got - want)
    print(
        f"  8e. a kernel the window truncates: column mass vs the table, "
    ...
    ue3 = _ugrid(2000)
    ref = (
        ZGammaLineshape(fsr=_toy_table(np.arange(50.0, 200.1, 0.125), ue3), **kw)
        .pdf(V)
        .numpy()
    )
    prev = None
    for dmn in (4.0, 2.0, 1.0, 0.5):
        p = (
            ZGammaLineshape(fsr=_toy_table(np.arange(50.0, 200.1, dmn), ue3), **kw)
            .pdf(V)
            .numpy()
        )
        d = np.max(np.abs(p - ref)) / np.max(ref)
        print(
            f"  8f. m nodes {dmn:4.2f} GeV apart: max rel dev {d:.3e}"
    ...
    pc = (
        ZGammaLineshape(
            fsr=_toy_table(np.arange(50.0, 200.1, 4.0), ue3), fsr_minterp="cubic", **kw
        )
        .pdf(V)
        .numpy()
    )
    print(
        f"  8f. the same 4 GeV nodes, cubic: max rel dev "
    ...
    for nc in (500, 1000, 2000, 4000):
        ...
        d = np.max(np.abs(p - ref)) / np.max(ref)
        print(
            f"  8g. {nc:5d} cells: max rel dev {d:.3e}"
            + (f"  (x{prev / d:.2f} finer)" if prev else "")
        )
    ...
    print(
        f"  8h. inline config -> JSON -> from_config: max abs dev {d:.2e} "
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        d = np.max(
            np.abs(ZGammaLineshape.from_config(cfg).pdf(V).numpy() - zp.pdf(V).numpy())
        )
        print(
            f"  8h. by reference: config is {len(json.dumps(cfg))} bytes, "
            f"max abs dev {d:.2e}"
        )
        ok &= d == 0.0
