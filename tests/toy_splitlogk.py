...
w = 1.0 + 0.15 * (x - 0.5)
upfolds = np.stack([rng.poisson(np.maximum(folds[f] * w, 0.0)) for f in range(K)], axis=0)
...
if __name__ == "__main__":
    ...
    print(f"wrote {out}/toy_splitlogk.hdf5 (split) and toy_sharedlogk.hdf5 (shared), K={K}")
